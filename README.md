...
- слушает `:80` (`web`) и `:443` (`websecure`) — единственный сервис с published-портами на хосте.
- `web` → авто-редирект на `websecure` (HTTP always redirects to HTTPS).
- TLS: Let's Encrypt через HTTP-challenge на entrypoint `web`, email из `${ACME_EMAIL}`, сертификаты в volume `letsencrypt`.
...
**Роутинг** — у каждого сервиса свой router/middleware/service (имя = имя сервиса, чтобы не перезаписать чужой), схема одна и та же: `Host(${DOMAIN}) && PathPrefix(/<prefix>)` → `stripprefix(/<prefix>)` → внутренний порт контейнера. `entrypoints=websecure`, TLS через `letsencrypt` resolver, `loadbalancer.server.port` — внутренний порт, наружу не `exposed` (`expose`, не `ports`).

Итог по маршрутам:

| Снаружи | Внутри | Куда |
|---|---|---|
| `https://${DOMAIN}/db/health` | `GET /health` | work-db |
| `https://${DOMAIN}/db/tickets` | `GET /tickets` | work-db |
| `https://${DOMAIN}/db/tickets/:id` | `GET /tickets/:id` | work-db |
| `https://${DOMAIN}/mcp/db/health` | `GET /health` | work-db-mcp |
| `https://${DOMAIN}/mcp/db/` | `POST /` (MCP JSON-RPC) | work-db-mcp |
| `https://${DOMAIN}/` (без префикса) | не матчится ни одним роутером → 404 у Traefik | — |
